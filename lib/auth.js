...
export async function getCurrentUser(){
  ...
  let rows;
  if(await tenancyEnabled())rows=await sql`select u.id,u.email,u.name,m.role,s.organization_id,o.name organization_name,w.id warehouse_id,w.name warehouse_name from sessions s join users u on u.id=s.user_id join organizations o on o.id=s.organization_id join organization_members m on m.organization_id=s.organization_id and m.user_id=u.id and m.active=true left join lateral(select id,name from warehouses where organization_id=o.id and active=true order by created_at limit 1) w on true where s.token_hash=encode(digest(${token},'sha256'),'hex') and s.expires_at>now() and u.active=true and o.status in ('ACTIVE','TRIAL') limit 1`;
  else rows=await sql`select u.id,u.email,u.name,u.role from sessions s join users u on u.id=s.user_id where s.token_hash=encode(digest(${token},'sha256'),'hex') and s.expires_at>now() and u.active=true limit 1`;
  return rows[0]||null;
}
