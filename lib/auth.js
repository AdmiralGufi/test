...
export async function createSession(userId){
  ...
  c.set(COOKIE,rows[0].token,{httpOnly:true,secure:true,sameSite:'lax',path:'/',maxAge:2592000});
}
...
export async function logout(){const c=await cookies();const token=c.get(COOKIE)?.value;if(token)await sql`delete from sessions where token_hash=encode(digest(${token},'sha256'),'hex')`;c.set(COOKIE,'',{httpOnly:true,secure:true,sameSite:'lax',path:'/',maxAge:0})}
