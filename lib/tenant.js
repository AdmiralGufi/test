...
export async function requireWarehouse(user,warehouseId){
  const organizationId=requireOrganization(user);
  const rows=await sql`select id from warehouses where id=${warehouseId} and organization_id=${organizationId} limit 1`;
  if(!rows[0])missing();
  return rows[0];
}
...
export async function requirePick(user,orderItemId,boxId){
  const organizationId=requireOrganization(user);
  const rows=await sql`select oi.id from order_items oi join orders o on o.id=oi.order_id join sellers os on os.id=o.seller_id join boxes b on b.id=${boxId} join sellers bs on bs.id=b.seller_id join zones bz on bz.id=b.zone_id where oi.id=${orderItemId} and os.organization_id=${organizationId} and bs.organization_id=${organizationId} and o.seller_id=b.seller_id and o.warehouse_id=bz.warehouse_id limit 1`;
  if(!rows[0])missing();
}
