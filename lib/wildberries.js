...
async function importOrder(integration,raw,actorId){
  ...
  const rows=await sql`with existing_product as (
      ...
      insert into orders(order_no,wb_order_id,seller_id,warehouse_id,status,priority)
      values(${order.orderNo},${order.id},${integration.seller_id},${integration.warehouse_id},'NEW','HIGH')
