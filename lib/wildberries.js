...
import {organizationUsage} from './plans.js';
...
export function normalizeWbOrder(order){
  ...
  const offices=Array.isArray(order.offices)?order.offices.map(value=>String(value||'').trim()).filter(Boolean):[];
  const officeId=Number(order.officeId)||null;
  const warehouseId=Number(order.warehouseId)||null;
  return {
    id,
    orderNo:`WB-${id}`,
    ...
    nmId,
    chrtId,
    destination:offices.join(' · ')||(officeId?`Пункт Wildberries #${officeId}`:null),
    officeId,
    warehouseId,
    orderUid:String(order.orderUid||'').trim()||null,
    sellerDate:normalizeDate(order.sellerDate),
    raw:order
  };
}
...
async function importOrder(integration,raw,actorId){
  ...
  const rows=await sql`with existing_product as (
      ...
      insert into orders(order_no,wb_order_id,seller_id,warehouse_id,status,priority,destination,wb_office_id,wb_warehouse_id,wb_order_uid,wb_seller_date)
      values(${order.orderNo},${order.id},${integration.seller_id},${integration.warehouse_id},'NEW','HIGH',${order.destination},${order.officeId},${order.warehouseId},${order.orderUid},${order.sellerDate})
    ...
    ), available_inventory as materialized (
      select i.id,i.cell_id from inventory i join zones z on z.id=i.zone_id cross join created_item ci
      where i.product_id=ci.product_id and z.warehouse_id=${integration.warehouse_id} and z.code='STG' and i.cell_id is not null
        and (i.physical_qty-i.reserved_qty-i.damaged_qty-i.quarantine_qty)>0
      order by i.updated_at,i.id limit 1 for update of i
    ), reserved as (
      update inventory i set reserved_qty=i.reserved_qty+1,updated_at=now()
      from available_inventory a where i.id=a.id returning a.cell_id
    ), allocated as (
      insert into order_allocations(order_item_id,cell_id,qty)
      select ci.id,r.cell_id,1 from created_item ci cross join reserved r returning id
    ), audit as (
      insert into audit_logs(actor_id,action,entity_type,entity_id,new_data)
      select ${actorId||integration.created_by},'WB_IMPORT_ORDER','order',created_order.id,jsonb_build_object('wb_order_id',${order.id}::text,'nm_id',${order.nmId}::bigint,'chrt_id',${order.chrtId}::bigint,'source','Wildberries FBS','stock_reserved',exists(select 1 from allocated))
...
export async function syncWildberriesIntegration(integration,actorId=null){
  ...
  try{
    ...
    const usage=await organizationUsage(integration.organization_id);
    const monthlyLimit=usage.limits.monthlyOrders;
    let remaining=monthlyLimit===null?Number.POSITIVE_INFINITY:Math.max(0,monthlyLimit-usage.monthly_orders);
    let imported=0;
    for(const order of orders){
      if(remaining<=0)break;
      if(await importOrder(integration,order,actorId)){imported+=1;remaining-=1}
    }
    const limited=remaining<=0&&orders.length>imported;
    await sql`update wb_integrations set last_sync_at=${startedAt},last_success_at=now(),last_error=${limited?'Достигнут месячный лимит заказов текущего тарифа':null},imported_orders=imported_orders+${imported},updated_at=now() where id=${integration.id}`;
    return {received:orders.length,imported,limited};
  }catch(error){
