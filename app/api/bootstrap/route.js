...
export async function GET(){
  ...
  const [warehouses,zones,cells,sellers,products,boxes,boxItems,orders,orderItems,tasks,devices,audit,users,organizations,integrations,subscription,warehouseStats]=await Promise.all([
    ...
    organizationId?sql`select bi.*,p.sku,p.name product_name,b.box_code,c.code cell_code,z.warehouse_id from box_items bi join products p on p.id=bi.product_id join boxes b on b.id=bi.box_id join sellers s on s.id=b.seller_id join zones z on z.id=b.zone_id left join cells c on c.id=b.cell_id where s.organization_id=${organizationId} order by b.received_at desc limit 5000`:sql`select bi.*,p.sku,p.name product_name,b.box_code,c.code cell_code,z.warehouse_id from box_items bi join products p on p.id=bi.product_id join boxes b on b.id=bi.box_id join zones z on z.id=b.zone_id left join cells c on c.id=b.cell_id order by b.received_at desc limit 5000`,
    ...
    organizationId?sql`select oi.*,p.sku,p.name product_name,c.code source_cell_code,b.box_code picked_box_code,coalesce(a.allocated_qty,0)::int allocated_qty from order_items oi join orders o on o.id=oi.order_id join sellers s on s.id=o.seller_id join products p on p.id=oi.product_id left join cells c on c.id=oi.source_cell_id left join boxes b on b.id=oi.picked_box_id left join lateral(select sum(oa.qty)::int allocated_qty from order_allocations oa where oa.order_item_id=oi.id) a on true where s.organization_id=${organizationId} order by oi.id limit 5000`:sql`select oi.*,p.sku,p.name product_name,c.code source_cell_code,b.box_code picked_box_code,coalesce(a.allocated_qty,0)::int allocated_qty from order_items oi join products p on p.id=oi.product_id left join cells c on c.id=oi.source_cell_id left join boxes b on b.id=oi.picked_box_id left join lateral(select sum(oa.qty)::int allocated_qty from order_allocations oa where oa.order_item_id=oi.id) a on true order by oi.id limit 5000`,
