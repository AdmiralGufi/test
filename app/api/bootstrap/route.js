...
export async function GET(){
  ...
  const [zones,cells,sellers,products,boxes,boxItems,orders,orderItems,tasks,devices,audit,users]=await Promise.all([
    ...
    sql`select bi.*,p.sku,p.name product_name,b.box_code,c.code cell_code from box_items bi join products p on p.id=bi.product_id join boxes b on b.id=bi.box_id left join cells c on c.id=b.cell_id order by b.received_at desc limit 5000`,
    sql`select o.*,s.name seller_name from orders o join sellers s on s.id=o.seller_id order by o.created_at desc limit 1000`,
    sql`select oi.*,p.sku,p.name product_name,c.code source_cell_code,b.box_code picked_box_code from order_items oi join products p on p.id=oi.product_id left join cells c on c.id=oi.source_cell_id left join boxes b on b.id=oi.picked_box_id order by oi.id limit 5000`,
    ...
    user.role==='ADMIN'?sql`select id,email,name,role,active,created_at from users order by created_at`:Promise.resolve([])
  ]);
  return NextResponse.json({loading:false,user,data:{zones,cells,sellers,products,boxes,boxItems,orders,orderItems,tasks,devices,audit,users}})
}
