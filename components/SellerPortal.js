...
export default function SellerPortal({user,data,onRefresh,onLogout}){
  ...
  return <div className="sellerApp">
    ...
    <main className="sellerContent">
      ...
      {tab==='Wildberries'&&<SellerWildberries user={user} seller={data.sellers?.[0]} warehouses={warehouses} integrations={data.integrations||[]} done={refreshed}/>}
    </main>
...
function SellerWildberries({user,seller,warehouses,integrations,done}){
  ...
  return <div className="wbSellerGrid"><section className="sellerPanel wbSellerIntro"><span className="wbBig">WB</span><p>WILDBERRIES FBS</p><h2>Заказы прямо в кабинет фулфилмента.</h2><div className="wbSteps"><div><b>01</b><span>Введите Marketplace-токен</span></div><div><b>02</b><span>Система проверит доступ</span></div><div><b>03</b><span>Новые заказы загрузятся автоматически</span></div></div><small>Токен хранится на сервере в зашифрованном виде и никогда не показывается сотрудникам склада.</small></section>
    <section className="sellerPanel wbSellerControl">{integration?<><div className="connectedBadge"><i/> ПОДКЛЮЧЕНО</div><h2>{seller?.name}</h2><p>{integration.name} · {integration.warehouse_name} · токен {integration.token_hint}</p><div className="syncStats"><div><small>Последнее обновление</small><b>{date(integration.last_success_at)}</b></div><div><small>Загружено заказов</small><b>{number(integration.imported_orders)}</b></div></div>{integration.last_error&&<div className="notice danger">Последняя синхронизация завершилась ошибкой. Повторите обновление.</div>}{error&&<div className="notice danger">{error}</div>}<button className="btn primary wide" disabled={Boolean(busy)} onClick={()=>call({action:'SYNC',integration_id:integration.id},result=>`Обновлено · новых заказов ${result.imported||0}`)}>{busy?'Обновляем…':'Обновить заказы сейчас'}</button>{canEdit&&<button className="linkButton dangerLink disconnectSeller" onClick={()=>call({action:'DISCONNECT',integration_id:integration.id},()=>`Wildberries отключён`)}>Отключить интеграцию</button>}</>:canEdit?<form onSubmit={event=>{event.preventDefault();call({action:'SAVE',seller_id:seller?.id,warehouse_id:warehouseId,token,name:'Wildberries FBS'},result=>`Wildberries подключён · загружено ${result.sync?.imported||0}`)}}><div className="connectedBadge pending">НАСТРОЙКА</div><h2>Подключить магазин</h2><p>В WB Партнёры откройте «Профиль → Интеграции по API» и создайте токен категории «Маркетплейс».</p><label className="field"><span>Склад обработки</span><select value={warehouseId} onChange={event=>setWarehouseId(event.target.value)} required><option value="">Выберите склад</option>{warehouses.map(warehouse=><option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></label><label className="field"><span>API-токен Wildberries<small>Не менее 20 символов</small></span><input type="password" autoComplete="off" value={token} onChange={event=>setToken(event.target.value)} placeholder="Вставьте полный токен" required/></label>{error&&<div className="notice danger">{error}</div>}<button className="btn primary wide" disabled={busy||!warehouseId||token.length<20}>{busy?'Проверяем и подключаем…':'Подключить Wildberries'}</button></form>:<><div className="connectedBadge pending">НЕТ ПОДКЛЮЧЕНИЯ</div><h2>Wildberries ещё не подключён</h2><p>Попросите владельца клиентского кабинета добавить API-токен.</p></>}</section></div>;
}
