...
function WildberriesIntegration({data,role,done}){
  const[form,setForm]=useState({seller_id:'',warehouse_id:data.warehouses?.[0]?.id||'',token:'',name:'Wildberries FBS'});
  const[busy,setBusy]=useState('');
  const[error,setError]=useState('');
  ...
  async function save(event){
    ...
    try{const result=await api('/api/integrations/wildberries',{method:'POST',body:JSON.stringify({action:'SAVE',...form})});setForm(current=>({...current,seller_id:'',token:'',name:'Wildberries FBS'}));await done(`Wildberries подключён · загружено заказов: ${result.sync?.imported||0}`)}catch(err){setError(err.message)}finally{setBusy('')}
  }
...
function PickLine({item,data,role,done}){
  ...
  const order=data.orders?.find(value=>value.id===item.order_id);
  const candidates=(data.boxItems||[]).filter(x=>x.product_id===item.product_id&&x.qty>0&&x.warehouse_id===order?.warehouse_id);
...
function Warehouses({data,done}){
  ...
  const[create,setCreate]=useState({code:'',name:'',city:'',address:'',timezone:'Asia/Bishkek'});
  ...
  async function createWarehouse(event){event.preventDefault();if(busy)return;setBusy(true);setError('');try{await ops({action:'CREATE_WAREHOUSE',...create});setCreate({code:'',name:'',city:'',address:'',timezone:'Asia/Bishkek'});await done('Новый склад и рабочие зоны созданы')}catch(err){setError(err.message)}finally{setBusy(false)}}
...
function Reports({data}){
  ...
  const shortages=(data.orderItems||[]).filter(item=>(item.allocated_qty||0)<item.qty&&item.picked_qty<item.qty).length;
  const integrations=data.integrations||[];
  return <div className="reportGrid"><section className="card"><SectionTitle title="Операционные KPI" text="Живая сводка по текущему фулфилменту"/><div className="reportMetrics"><article><small>Активные заказы</small><b>{active}</b><span>в обработке</span></article><article><small>Отгружено</small><b>{shipped}</b><span>в загруженной истории</span></article><article><small>Просрочено</small><b className={late?'dangerText':''}>{late}</b><span>требуют внимания</span></article><article><small>Без полного резерва</small><b className={shortages?'dangerText':''}>{shortages}</b><span>позиций к пополнению</span></article><article><small>Доступный остаток</small><b>{available}</b><span>из {physical} единиц</span></article></div></section><section className="card"><SectionTitle title="Склады" text="Остатки и резервы по площадкам"/><DataTable heads={['Склад','Город','Физически','Резерв','Доступно','QC / брак']} empty="Нет активных складов" rows={(data.warehouseStats||[]).map(item=>[<b>{item.name}</b>,item.city||'—',item.physical_qty,item.reserved_qty,item.available_qty,(item.quarantine_qty||0)+(item.damaged_qty||0)])}/></section><section className="card"><SectionTitle title="Контроль Wildberries" text="Состояние автоматической загрузки FBS"/><DataTable heads={['Клиент','Склад','Последний успех','Заказов','Состояние']} empty="Wildberries ещё не подключён" rows={integrations.map(item=>[<b>{item.seller_name}</b>,item.warehouse_name||'—',item.last_success_at?new Date(item.last_success_at).toLocaleString('ru-RU'):'—',item.imported_orders||0,item.last_error?<Status value="Требует внимания" tone="CANCELLED"/>:<Status value="Работает" tone="READY"/>])}/></section><section className="card"><SectionTitle title="Выгрузки для отчётности" text="CSV открывается в Excel и Google Таблицах"/><div className="exportButtons"><a className="btn" href="/api/export?type=orders">Заказы CSV</a><a className="btn" href="/api/export?type=inventory">Остатки CSV</a><a className="btn" href="/api/export?type=boxes">Короба CSV</a><a className="btn" href="/api/export?type=products">Товары CSV</a></div></section></div>;
}
